...
const SAVED_DRAFTS_KEY = 'diy-bag-saved-designs-v1'
...
export const useEditorStore = defineStore('editor', () => {
  ...
  function addPattern(patternId: string): void { const pattern = getPattern(patternId); if (!canPlacePattern(pattern, bag.value.embroideryArea)) return; const item = { id: placementId(), patternId, centerXRatio: 0.5, centerYRatio: 0.5, rotationDegrees: 0 }; placements.value.push(clampPlacement(item, pattern, bag.value.embroideryArea)); selectedPlacementId.value = item.id }
  ...
  function rotateSelected(deltaDegrees: number): void { if (!selectedPlacement.value) return; updatePlacement({ ...selectedPlacement.value, rotationDegrees: selectedPlacement.value.rotationDegrees + deltaDegrees }) }
  ...
  async function loadCatalog(bagId?: string): Promise<BagDefinition[]> { loadingCatalog.value = true; catalogError.value = ''; try { const [bags, apiCategories, apiPatterns, limit] = await Promise.all([catalogApi.bags(), catalogApi.categories(), catalogApi.patterns(), catalogApi.limit()]); maxDrafts.value = limit.max_drafts; const choices = bags.map(toBag); if (!choices.length) throw new Error('暂无已上架且已配置刺绣区域的包包'); categories.value = apiCategories.map((item) => ({ id: item.id, name: item.name })); catalog.value = apiPatterns.map((item) => ({ id: item.id, categoryId: item.category_id, name: item.name, imageUrl: `http://localhost:8000${item.image_url}`, width: item.width_mm, height: item.height_mm, priceCents: item.price_cents, patternVersionId: item.pattern_version_id })); bag.value = choices.find((item) => item.id === bagId) ?? choices[0]; activeCategoryId.value = categories.value[0]?.id ?? ''; placements.value = placements.value.filter((placement) => catalog.value.some((pattern) => pattern.id === placement.patternId)); catalogLoaded.value = true; return choices } catch (error) { catalogError.value = error instanceof Error ? error.message : '加载商品数据失败'; return [] } finally { loadingCatalog.value = false } }
  async function saveDesign(): Promise<void> { if (savedDesigns.value.length >= maxDrafts.value) throw new Error(`最多保存 ${maxDrafts.value} 个设计，请先删除一个`); const items = placements.value.map((item) => { const pattern = getPattern(item.patternId); if (!pattern.patternVersionId) throw new Error('当前图案不是正式版本，无法保存'); return { pattern_version_id: pattern.patternVersionId, center_x_ratio: item.centerXRatio, center_y_ratio: item.centerYRatio, rotation_degrees: item.rotationDegrees } }); const design = await catalogApi.saveDesign({ bag_id: bag.value.id, client_key: clientKey(), items }); savedDesign.value = { id: design.id, totalPriceCents: design.total_price_cents }; savedDesigns.value.unshift({ ...savedDesign.value, bagId: bag.value.id, placements: JSON.parse(JSON.stringify(placements.value)) }); }
  function restoreDesign(design: StoredDesign): void { placements.value = JSON.parse(JSON.stringify(design.placements)); selectedPlacementId.value = null; savedDesign.value = { id: design.id, totalPriceCents: design.totalPriceCents } }
  ...
  watch(placements, (value) => uni.setStorageSync(DRAFT_KEY, value), { deep: true }); watch(savedDesigns, (value) => uni.setStorageSync(SAVED_DRAFTS_KEY, value), { deep: true })
  return { bag, categories, activeCategoryId, activePatterns, placements, selectedPlacementId, selectedPlacement, patternPriceCents, totalPriceCents, catalogError, loadingCatalog, catalogLoaded, savedDesign, savedDesigns, maxDrafts, getPattern, addPattern, updatePlacement, selectPlacement, deleteSelected, replaceSelected, rotateSelected, clearDraft, loadCatalog, saveDesign, restoreDesign, removeSavedDesign }
})
interface StoredDesign { id: string; totalPriceCents: number; bagId: string; placements: PatternPlacement[] }
function readSavedDesigns(): StoredDesign[] { try { const value = uni.getStorageSync(SAVED_DRAFTS_KEY); return Array.isArray(value) ? value as StoredDesign[] : [] } catch { return [] } }
